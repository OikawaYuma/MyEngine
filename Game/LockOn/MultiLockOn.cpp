#include "MultiLockOn.h"
#include "Input.h"
#include "TextureManager.h"
#include <cmath>
#include <cstdlib>
#include <utility>
void MultiLockOn::Init()
{
...
void MultiLockOn::Update(const std::list<std::unique_ptr<Enemy>>& enemies, Camera* camera, Player* player)
{
	if (!target_) {
		// ロックオンボタンをトリガーしたら
		if (player->GetBulletMode() == BulletMode::LaserBeam) {
			// ロックオン対象の検索
			// 目標
	...
	// ロックオン継続
		...
		for (std::list<std::unique_ptr<Sprite>>::iterator itr = lockOnMarks_.begin(); itr != lockOnMarks_.end(); itr++) {
			(*itr)->SetPosition({ pos.x,pos.y });
		}
...
void MultiLockOn::Draw()
{
	if (target_ != nullptr) {
		for (std::list<std::unique_ptr<Sprite>>::iterator itr = lockOnMarks_.begin(); itr != lockOnMarks_.end(); itr++) {
			(*itr)->Draw(lockOnMarkTex_, { 1.0f,1.0f,1.0f,1.0f });
		}
	}
}

Vector3 MultiLockOn::GetTargetPosition() const
{
	if (target_) {
...
bool MultiLockOn::OutRangeJudge(Camera* camera)
{
	// 敵のロックオン座標取得
	Vector3 positionWolrd = target_->GetWorldPosition();
