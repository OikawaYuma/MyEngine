...
PSOProperty Bloom::CreatePipelineStateObject()
{
	...
	property.pixelShaderBlob = CompileShader(L"Resources/shader/Bloom.PS.hlsl",
...
std::vector<D3D12_DESCRIPTOR_RANGE> Bloom::CreateDescriptorRange()
{
...
std::vector<D3D12_ROOT_PARAMETER> Bloom::CreateRootParamerter(std::vector<D3D12_DESCRIPTOR_RANGE>& descriptorRange)
{
...
std::vector<D3D12_STATIC_SAMPLER_DESC> Bloom::CreateSampler()
{
...
D3D12_ROOT_SIGNATURE_DESC Bloom::CreateRootSignature(PSOProperty& pso, std::vector<D3D12_ROOT_PARAMETER>& rootParameters, std::vector<D3D12_STATIC_SAMPLER_DESC>& samplers)
{
	HRESULT hr;
	// RootSignature作成
...
D3D12_INPUT_LAYOUT_DESC Bloom::SetInputLayout()
{
...
D3D12_BLEND_DESC Bloom::SetBlendState()
{
	// blendStateの設定
	//すべての色要素を書き込む
	// blendStateの設定
...
D3D12_RASTERIZER_DESC Bloom::SetRasterrizerState()
{
	// RasiterzerStateの設定
...
D3D12_DEPTH_STENCIL_DESC Bloom::CreateDepth()
{
