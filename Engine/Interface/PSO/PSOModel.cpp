...
void PSO::CreateRootSignature() {
	...
	descriptorRange_[1].BaseShaderRegister = 0; // 1から始まる
	...
	rootParamerters[1].DescriptorTable.pDescriptorRanges = &descriptorRange_[0]; // Tableの中身の配列を指定
	...
	rootParamerters[2].DescriptorTable.pDescriptorRanges = &descriptorRange_[1]; // Tableの中身の配列を指定
